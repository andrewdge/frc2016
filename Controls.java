...
public final class Controls{
	...
	private static final int XBOX_BUTTON_LBUTTON = 5;
	private static final int XBOX_BUTTON_RBUTTON = 6;
    ...
    //negate joystick values for forward and backwards
    ...
    public static double intakeRollerThrottle(){
        return deadband(xboxController.getRawAxis(XBOX_AXIS_LTHUMB_Y));
    }

    public static double intakeLiftThrottle(){
        if(xboxController.getRawButton(XBOX_BUTTON_LBUTTON)){
            return -0.8;
        }else if(xboxController.getRawButton(XBOX_BUTTON_RBUTTON)){
            return 0.4;
        }else return 0;
	}

	public static boolean shooterEnable(){
		return xboxController.getRawButton(XBOX_BUTTON_A);
	}
	
	public static int getShooterTopRPM(){
		return (int)SmartDashboard.getNumber("Top Shooter", 1000);
	}
	
	public static int getShooterBottomRPM(){
		return (int)SmartDashboard.getNumber("Bottom Shooter", 1000);
	}
}
